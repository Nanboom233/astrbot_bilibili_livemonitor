...
class BilibiliLiveMonitor(Star):
    def __init__(self, context: Context, config: AstrBotConfig):
        ...
        import threading
        threading.Thread(target=self.monitor_task, daemon=True).start()

    def get_room_init(self, room_id):
        try:
    ...
    def check_live_status(self, room_id):
        try:
            init_data = self.get_room_init(room_id)
            if not init_data:
                return None
            
            self.room_status[room_id]["last_check_time"] = datetime.now()
            room_data = self.get_room_info(room_id)
    ...
    def download_cover(self, cover_url, room_id):
        if not cover_url:
            return None
        save_dir = "covers"
        ...
        try:
            ...
            logger.error(f"下载直播间{room_id}封面失败: {str(e)}")
            return None
        ...
        while self.running:
            try:
                for room_id, _ in self.room_ids:
                    status_data = self.check_live_status(room_id)
                    if not status_data:
                        continue
                    ...
                    if current_status != last_status:
                        ...
                        if current_status == 1:
                            if not has_sent_notice:
                                ...
                                save_path = self.download_cover(cover_url, room_id)
                                ...
                                for group_id in list(set(self.groups)):
                                    try:
                                        self.context.send_message(group_id, message).result()
    ...
    def get_live_info(self, room_id=None):
    ...
    async def liveinfo_command(self, event: AstrMessageEvent, room_id: str = None):
        info = self.get_live_info(room_id)
    ...
    def terminate(self):
        ...
        try:
            ...
            logger.error(f"关闭requests会话失败: {str(e)}")
